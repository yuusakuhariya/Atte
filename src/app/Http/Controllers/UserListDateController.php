...
class UserListDateController extends Controller
{
    public function userListDate($id)
    {
            ...
            foreach ($users as $user)
            {
                ...
                foreach ($attendances as $attendance) {    // 複数あるレコードの入っている$attendancesを1つずつのレコード$attendanceにする。
                    ...
                    if ($start_time && $end_time) {    // もしstart_time（出勤）とend_time（退勤）が存在するなら、
                        $total_work_time = strtotime($end_time) - strtotime($start_time);    // 退勤の時間から出勤の時間を減算し、差分を秒で出す。
                        // strtotime() とは、人間が読み取り可能な日付文字列をUnixタイムスタンプに変換するために使用する。（秒数に変換）
                        $work_time = gmdate('H:i:s', $total_work_time);    // $total_timeを時・分・秒に変換し、$work_timeに代入する。
                    ...
                    foreach ($rests as $rest) {
                        ...
                        $user_rest_times[$attendance->id] = gmdate('H:i:s', $total_rest_time);
                    }
                }
                // dd($users);
            }
        ...
        return view('userListDate', compact('users', 'attendances', 'work_times','user_rest_times'));
    }
}
